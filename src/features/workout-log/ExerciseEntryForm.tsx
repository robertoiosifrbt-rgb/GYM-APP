...
function entrySetsToDraft(entry?: WorkoutEntry): DraftSet[] {
  if (!entry) return [{}]
  return entry.sets.map((set) => Object.fromEntries(Object.entries(set).map(([fieldId, value]) => [fieldId, String(value)])))
}

export function ExerciseEntryForm({ exercises, fieldTypes, getLastEntry, initialEntry, onAdd, onUpdate, onCancel }: ExerciseEntryFormProps) {
  ...
  function labelFor(fieldId: string) { return fieldTypes.find((f) => f.id === fieldId)?.label ?? fieldId }
  ...
  return (
    <form className="exercise-entry-form" onSubmit={handleSubmit}>
      <div className="field field-wide">
        <label htmlFor={editing ? `exercise-select-${initialEntry?.id}` : 'exercise-select'}>Exercise</label>
        <select id={editing ? `exercise-select-${initialEntry?.id}` : 'exercise-select'} value={exerciseId} onChange={(e) => handleExerciseChange(e.target.value)} required>
          <option value="" disabled>Select exercise</option>
          {exercises.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}
        </select>
      </div>

      {lastEntry && <p className="last-log-hint">Last time ({lastEntry.date}): {lastEntry.sets.map((set) => formatSet(set, fieldTypes)).join(', ')}</p>}
        ...
        <div className="sets-list">
          <div className="sets-header" aria-hidden="true">
            ...
            {exercise.fields.map((fieldId) => {
              ...
              return field ? <span key={fieldId}>{field.label}</span> : null
            })}
            <span />
          </div>
          {sets.map((set, index) => (
            <div className="set-row" key={index}>
              ...
              {exercise.fields.map((fieldId) => {
                ...
                return <input key={fieldId} aria-label={`Set ${index + 1} ${field.label}`} inputMode="decimal" type="number" step={0.1} min={SET_VALUE_BOUNDS.min} max={SET_VALUE_BOUNDS.max} placeholder="—" value={set[fieldId] ?? ''} onChange={(e) => updateSetField(index, fieldId, e.target.value)} />
              })}
              <button type="button" className="set-remove" onClick={() => removeSetRow(index)} aria-label={`Remove set ${index + 1}`} disabled={sets.length === 1}>×</button>
            </div>
          ))}
          <button type="button" className="add-set-button" onClick={addSetRow}>+ Add set</button>
        </div>
      )}

      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="form-actions">
        <button type="submit" disabled={!exercise}>{editing ? 'Save exercise' : 'Log exercise'}</button>
        {editing && onCancel && <button type="button" onClick={onCancel}>Cancel</button>}
      </div>
    </form>
  )
}
