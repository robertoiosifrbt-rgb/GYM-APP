 ...
 return <div className="exercise-card-list">{exercises.map(exercise=><article className={`exercise-card card ${detailId===exercise.id?'exercise-card-detail-open':''}`} key={exercise.id}>
  {editingId===exercise.id?<ExerciseForm exercises={exercises} fieldTypes={fieldTypes} onAddFieldType={onAddFieldType} onRemoveFieldType={onRemoveFieldType} initial={exercise} submitLabel="Save changes" onSubmit={(name,fields,details)=>{if(!onUpdate(exercise.id,name,fields,details))return false;setEditingId(null);return true}} onCancel={()=>setEditingId(null)}/>:<>
   ...
   {detailId===exercise.id&&<section className="exercise-target-detail full-detail">
    <div className="exercise-detail-hero"><div className="exercise-detail-hero-top"><button type="button" aria-label="Close exercise details" onClick={()=>setDetailId(null)}>‹</button><strong>{exercise.name}</strong><span aria-hidden="true">☆</span></div><div className="exercise-visual"><ExerciseMuscleMap primaryMuscles={exercise.primaryMuscles} secondaryMuscles={exercise.secondaryMuscles} exerciseName={exercise.name}/><div className="exercise-visual-copy"><strong>{exercise.name}</strong><span>{exercise.primaryMuscles||exercise.category||'Exercise'}</span></div></div></div>
    ...
    {detailTab==='instructions'?<div className="exercise-detail-instructions">{(exercise.instructions||'No instructions added yet.').split(/\n+/).filter(Boolean).map((line,index)=><p key={`${line}-${index}`}><span>{index+1}</span>{line}</p>)}</div>:<div className="exercise-muscle-panel"><ExerciseMuscleMap primaryMuscles={exercise.primaryMuscles} secondaryMuscles={exercise.secondaryMuscles} exerciseName={exercise.name}/><div><span>Primary</span><strong>{exercise.primaryMuscles||'Not specified'}</strong></div><div><span>Secondary</span><strong>{exercise.secondaryMuscles||'Not specified'}</strong></div></div>}
