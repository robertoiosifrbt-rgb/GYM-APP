...
- `src/features/exercises/` — biblioteca de exerciții, goală la început, completată de utilizator. Fiecare exercițiu are un nume și bifează ce urmărește — implicit `Reps / Weight (kg) / Time (s) / Distance (m)` (`DEFAULT_FIELD_TYPES` în `types.ts`), dar utilizatorul poate adăuga propriile categorii (nume + unitate opțională) prin butonul "+ Add" din `ExerciseForm`, persistate prin `useFieldTypes`. Combinație liberă per exercițiu — plank are doar Time, un exercițiu de forță are Reps + Weight. Persistență în `localStorage` (`useExercises` pentru exerciții, `useFieldTypes` pentru categoriile disponibile).
- `src/features/workout-log/` — jurnal zilnic de antrenament: alegi un exercițiu existent (din `exercises`), apoi adaugi seturi — câmpurile din formular se adaptează automat la ce urmărește exercițiul respectiv (`ExerciseEntryForm`). La selectarea exercițiului, apare automat ultimul log salvat pentru el (`getLastEntry` în `useWorkoutLog`), ca reper pentru progres. Formatarea seturilor pentru afișare e centralizată în `formatSet.ts`. Persistență în `localStorage`. Istoric grupat pe dată în `WorkoutHistory`.
