...
- `src/features/workout-log/` — jurnal de antrenament, organizat pe **sesiuni**, nu direct pe exerciții izolate. O sesiune (`WorkoutSession` în `types.ts`, `useWorkoutSessions`) are o dată și un nume opțional (ex: "Push Day"). `SessionPicker` selectează automat sesiunea de azi dacă există; altfel arată direct formularul de creare. Nu are un `<select>`/căutare cu toate sesiunile (nu scalează la sute) — ca să continui o sesiune mai veche, apeși direct pe titlul ei din `WorkoutHistory` (heading apăsabil), care o face sesiunea curentă. Exercițiile se adaugă în sesiunea curentă (`ExerciseEntryForm` — alegi exercițiul din `exercises`, câmpurile de set se adaptează automat la ce urmărește acel exercițiu). La selectarea exercițiului apare automat ultimul log salvat pentru el (`getLastEntry` în `useWorkoutLog`), ca reper de progres. Formatarea seturilor e centralizată în `formatSet.ts`. Sesiunile sunt editabile ("Edit" în `SessionPicker`, pe sesiunea curentă) — dacă se schimbă data, `updateEntriesDate` sincronizează data pe toate intrările legate de acea sesiune, ca istoricul să rămână corect. Persistență în `localStorage` (sesiuni și intrări separat). Istoric grupat pe sesiune în `WorkoutHistory`.
