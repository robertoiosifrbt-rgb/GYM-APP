...
- `src/features/exercises/` — biblioteca de exerciții, goală la început, completată de utilizator. Fiecare exercițiu are un nume și bifează ce urmărește dintre `Reps / Weight (kg) / Time (s) / Distance (m)` (`SET_FIELDS` în `types.ts`) — combinație liberă, ca un exercițiu de tip plank să aibă doar Time, iar unul de forță Reps + Weight. Persistență în `localStorage` prin `useExercises`.
