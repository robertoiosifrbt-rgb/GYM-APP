...
- Ecranul negru a persistat și doar la navigarea pe tab-ul Photos (fără nicio poză selectată). Adăugat `ErrorBoundary` (`src/app/ErrorBoundary.tsx`) care afișează eroarea pe ecran în loc de gol/negru — a scos la iveală cauza reală: o intrare veche în IndexedDB, salvată înainte de schimbarea la 4 poze/dată, fără câmpul `photos`. Fix: `usePhotos` filtrează și șterge automat intrările incompatibile cu formatul curent (`isValidPhotoSet` în `types.ts`).
