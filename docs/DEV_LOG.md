...
- Intrările vechi rămâneau doar "vizibile" în istoric, fără sesiune reală de continuat (nu apăreau în `SessionPicker`). Adăugat migrare automată, o singură dată la încărcare (`WorkoutLogPage`): pentru fiecare dată veche fără sesiune, se creează o sesiune reală (fără nume) și intrările sunt legate de ea (`backfillSessionIds`) — devin alegeri normale în dropdown, poți adăuga exerciții noi în ele.
