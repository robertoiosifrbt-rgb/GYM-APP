...
- Design: sistem de culori pe variabile CSS (light/dark), layout de aplicație mobilă (header sticky + bottom tab bar), carduri rotunjite, `viewport-fit=cover` pentru iOS. Bottom nav consolidat la 3 tab-uri (Home, Body, Workout), cu `SubNav` pentru navigare rapidă între sub-secțiuni (Measurements/Photos, Log/Exercises).
